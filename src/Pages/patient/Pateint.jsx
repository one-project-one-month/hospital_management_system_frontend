...
const Pateint = () => {
  const isPersonalFormComplete = useSelector(
    (state) => state.form.isFormComplete
  );
  ...
  const isContactFormComplete = useSelector(
    (state) => state.contact.isFormComplete
  );
  ...
  const isHealthProfileComplete = useSelector(
    (state) => state.healthProfile.isFormComplete
  );
  const isHealthProfileClick = useSelector(
    (state) => state.healthProfile.isClick
  );
  const healthProfile = useSelector(
    (state) => state.healthProfile.healthProfileInfo
  );
    ...
    if (Object.keys(combinedDatas).length !== 0) {
      try {
        const response = await toast.promise(
          axios.post(
            "https://hospital-management-system-backend-7fee.vercel.app/api/v1/patients",
            combinedDatas
          ),
          {
          ...
          >
            <div className="bg-white w-full max-h-full rounded-xl p-8">
              ...
              {isPersonalFormClick &&
                isPersonalFormComplete &&
                !isContactFormComplete &&
                !isHealthProfileComplete && <Contactinfo />}

              {isPersonalFormClick &&
                isPersonalFormComplete &&
                isContactFormClick &&
                isContactFormComplete && <HealthProfile />}
